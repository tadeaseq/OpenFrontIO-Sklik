...
var Path2D = class Path2D {
	...
	constructor() {
		borrowedFromCanvas.forEach((key) => {
			this[key] = vi.fn(CanvasRenderingContext2D$1.prototype[key].bind(this));
		});
...
var createImageBitmap_default = vi.fn(function createImageBitmap(img, sx, sy, sWidth, sHeight, options) {
...
function mockPrototype(win) {
	...
	*/
	const getContext2D = vi.fn(function getContext2d(type) {
		...
		try {
			if (!this.dataset.internalRequireTest) __require("canvas");
		} catch {
			return null;
		}
	...
	if (!vi.isMockFunction(htmlCanvasElementPrototype.getContext)) getContext2D.internal = htmlCanvasElementPrototype.getContext;
	...
	*/
	const toBlobOverride = vi.fn(function toBlobOverride$1(callback, mimetype) {
	...
	if (!vi.isMockFunction(htmlCanvasElementPrototype.toBlob)) toBlobOverride.internal = htmlCanvasElementPrototype.toBlob;
	...
	* jpeg size is variable, and so is png. TODO: Is there a better way to do this?
	*/
	const toDataURLOverride = vi.fn((type, _encoderOptions) => {
		switch (type) {
			case "image/jpeg": break;
			case "image/webp": break;
	...
	if (!vi.isMockFunction(htmlCanvasElementPrototype.toDataURL)) toDataURLOverride.internal = htmlCanvasElementPrototype.toDataURL;
