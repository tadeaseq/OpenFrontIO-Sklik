...
var resolve2 = isWindows ? (...paths) => normalize(path.win32.resolve(...paths)) : path.posix.resolve;
...
var src_default = (opts = {}) => {
  let projectRoot;
  let workspaceRoot;
  let hasTypeScriptDep;
  let processConfigFile;
  let invalidateConfigFile;
  let getResolvers = async function* () {
  };
  let watcher;
  let viteLogger;
  let directoryCache;
  ...
  const plugin = {
    ...
    configResolved(config) {
      ...
      let { root } = opts;
      if (root) {
        root = projectRoot = workspaceRoot = resolve2(config.root, root);
    ...
    async buildStart() {
      ...
      const addProject = (project, data) => {
        ...
        if (watcher) {
          ...
          (_a2 = project.extended) == null ? void 0 : _a2.forEach((parent) => {
      ...
      const loadProject = async (tsconfigFile, data) => {
        ...
        if (project) {
          ...
          watcher == null ? void 0 : watcher.add(tsconfigFile);
        }
      };
      ...
      if (opts.projects || opts.projectDiscovery !== "lazy") {
        const projectPaths = (_b = (_a = opts.projects) == null ? void 0 : _a.map((file) => {
          if (!file.endsWith(".json")) {
            file = join2(file, "tsconfig.json");
          }
          return resolve2(projectRoot, file);
        })) != null ? _b : await tsconfck.findAll(workspaceRoot, {
          configNames,
          skip: shouldSkipDir
        });
      ...
      getResolvers = async function* (importer) {
        ...
        while (dir !== (dir = dirname2(dir)) && dir !== root) {
          ...
          if (opts.projectDiscovery === "lazy") {
            ...
            await ((_a2 = data.lazyDiscovery) != null ? _a2 : data.lazyDiscovery = discoverProjects(dir, data));
          } else if (!data) {
            continue;
          }
    ...
    async resolveId(id, importer, options) {
      ...
      if (importer[0] === "\0") {
        ...
        if (index !== -1) {
          ...
          if (isAbsolute(query) && fs.existsSync(query)) {
